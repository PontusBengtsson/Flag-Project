...
const CountryCards = ({ countries, loading }) => {
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center',  backgroundColor: theme.palette.background.default }}>
      <Box sx={{ width: { xs: '340px', sm: '340px', md: '90%', lg: '1152px' } }}>
        <Grid container columnSpacing={4}>
          {countries
            .sort((a, b) => a.name.common.localeCompare(b.name.common))
            .map((country) => (
              <Grid item xs={12} sm={12} md={4} lg={3} xl={3} key={country.cca3} sx={{ marginBottom: 7, alignItems: 'center' }}>
                <Link to={`/country/${country.cca3}`} style={{ textDecoration: 'none' }}>
                  <Box sx={{ width: '100%', cursor: 'pointer', borderRadius: '4px', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: 'rgba(0, 0, 0, 0.2) 0px 3px 5px -1px, rgba(0, 0, 0, 0.14) 0px 6px 10px 0px, rgba(0, 0, 0, 0.12) 0px 1px 18px 0px', backgroundColor: theme.palette.background.paper }}>
                    {loading ? (
