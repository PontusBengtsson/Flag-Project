        ...
        <Grid container columnSpacing={4}>
          {isLoading
            ? [...Array(12)].map((_, index) => (
                <Grid item xs={12} sm={6} md={4} lg={3} key={index} sx={{ marginBottom: 7, marginTop: index < 4 ? 0 : 0 }}>
                  <Box sx={{ borderRadius: 2, boxShadow: 'none' }}>
                    <Skeleton variant="rectangular" width="100%" height={265} sx={{ borderRadius: '8px' }} />
                  </Box>
                </Grid>
              ))
            : countries
                ...
                .map((country) => (
                  <Grid item xs={12} sm={6} md={4} lg={3} key={country.cca3} sx={{ marginBottom: 7, marginTop: 0 }}>
